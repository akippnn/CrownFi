...
├── services/api/           # Rust/Axum API skeleton for platform refactor
├── infra/                  # Docker Compose topology for Postgres/Redis/API/web
├── packages/shared/        # Reserved for shared schemas/types during the refactor
...
│   ├── architecture/       # Current platform, component boundaries, refactor plan
│   ├── features/           # Voting, ticketing, verification, admin, collectibles
│   ├── blockchain/         # Stellar/Soroban and transaction verification notes
│   ├── setup/              # Local testing, Supabase, deployment, Compose paths
│   ├── security/           # Security audit notes
│   └── planning/           # Refactor TODOs
└── SECURITY.md             # Root security policy and reporting notes
```

> Refactor note: `web/` still contains the active MVP app. `services/api/` and `infra/` are the first step toward the platform split described in `docs/architecture/platform-refactor-plan.md`.
...
For a full walkthrough, use [`docs/setup/supabase.md`](docs/setup/supabase.md) and [`docs/demo/user-flow.md`](docs/demo/user-flow.md).

## Platform refactor quick check

The `refactor/platform-architecture` branch introduces a Rust/Axum API skeleton and Compose topology without removing the existing Next.js API routes yet.

Run only Postgres and Redis:
...
```bash
cd services/api
CROWNFI_API_BIND=127.0.0.1:8080 cargo run
```

Smoke test the API:

```bash
API_BASE_URL=http://localhost:8080 ./scripts/smoke-api.sh
```
...
See [`docs/setup/local-mvp-testing.md`](docs/setup/local-mvp-testing.md) and [`docs/setup/production-compose.md`](docs/setup/production-compose.md).
...
`cargo audit --deny warnings` currently reports advisory warnings from transitive Soroban/Arkworks dependencies. These are documented in [`docs/security/security-audit.md`](docs/security/security-audit.md) and are kept visible but non-blocking in CI.
...
This branch includes an initial MVP security hardening pass.
...
See [`SECURITY.md`](SECURITY.md) and [`docs/security/security-audit.md`](docs/security/security-audit.md) for details.
...
| Document | Purpose |
|---|---|
| [`docs/demo/user-flow.md`](docs/demo/user-flow.md) | Step-by-step fan/admin demo walkthrough |
| [`docs/setup/supabase.md`](docs/setup/supabase.md) | Supabase/Postgres setup |
| [`docs/operations/workflow.md`](docs/operations/workflow.md) | End-to-end workflow and system behavior |
| [`docs/setup/deployment.md`](docs/setup/deployment.md) | VPS/deployment entry point |
| [`docs/setup/production-compose.md`](docs/setup/production-compose.md) | Docker Compose production-like path |
...
| [`docs/architecture/current-platform.md`](docs/architecture/current-platform.md) | Architecture notes |
| [`docs/architecture/component-boundaries.md`](docs/architecture/component-boundaries.md) | UI component ownership rules for safe redesign/refactor work |
