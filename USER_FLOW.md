...
- A **Supabase** project (free) — see [`SUPABASE.md`](SUPABASE.md)
...
git clone <repo-url>
cd crownfi/web
cp .env.example .env          # then paste your Supabase DATABASE_URL + DIRECT_URL (see SUPABASE.md)
npm install
...
| Symptom | Fix |
|---|---|
| Page shows "Unexpected end of JSON input" / no data | DB not configured — check `web/.env`, see [`SUPABASE.md`](SUPABASE.md) |
