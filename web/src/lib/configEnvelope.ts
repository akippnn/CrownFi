...
function material(): Buffer {
  ...
  if (process.env.NODE_ENV === "production" && !isLocalProfile()) {
    throw new Error("CROWNFI_CONFIG_PROTECTION_KEY is required outside the local profile");
  }
...
export function sealConfiguration(value: unknown): string {
...
export function maskedSuffix(value: unknown): string | null {
